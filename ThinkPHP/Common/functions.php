...
//WriteLog To Mysql DB!
function writeLog($url,$param,$method,$userIp)
{
//    var_dump($param);
    $data = array(
        'uid'           =>session('userInfo')['fid'],
        'account'       =>session('userInfo')['faccount'],
    ...
    return D('operation_log')->add($data);
}
 ...
 * object = filesname
 */
function uploadFileToAliOSS($bucket,$object,$filePath){

    $ossClient = initOss();
    try{
        $ossClient->uploadFile($bucket, $object, $filePath);
    } catch(OssException $e) {
        writeLog('/developer/fileupload/upload',sprintf('bucket:%s,object:%s,filePath:%s,ErrorMsg:%s',$bucket,$object,$filePath,$e->getMessage()),'POST',$_SERVER['REMOTE_ADDR']);
        return false;
    }
    return true;
}
...
function getAliyunOSSBucketList()
{
    $bucket_list = array();
...
function getObjectListByBucketName($bucket)
{
    try {
        $ossClient = initOss();
...
function uploadFilesToAliyunOSS($path)
{
    // 获取配置项
